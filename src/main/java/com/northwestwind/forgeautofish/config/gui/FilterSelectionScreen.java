...
public class FilterSelectionScreen extends Screen {
    ...
    protected void init() {
        ...
        Button add = new Button(this.width / 2 - 75, 60, 72, 20, new TranslationTextComponent("gui.filterselection.save").getString(), button -> {
        ...
        addButton(add);
        Button done = new Button(this.width / 2 + 3, 60, 72, 20, new TranslationTextComponent("gui.filterselection.cancel").getString(), button -> Minecraft.getInstance().displayGuiScreen(parent));
        addButton(done);
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i = page * max; i < Math.min((page + 1) * max, searching.size()); i++) {
            ...
            if (!stack.isEmpty()) {
                ...
                if (selected.contains(item)) fillGradient(x - 2, y - 2, x + 18, y + 18, Color.GREEN.getRGB(), Color.GREEN.getRGB());
                else if (isMouseInRange(mouseX, mouseY, x, y,x + 16, y + 16)) fillGradient(x - 2, y - 2, x + 18, y + 18, Color.LIGHT_GRAY.getRGB(), Color.LIGHT_GRAY.getRGB());
                if (isMouseInRange(mouseX, mouseY, x, y,x + 16, y + 16)) renderTooltip(stack, mouseX, mouseY);
            }
